...
import { calculateCartTotal, calculateCartTotalMinus } from '@/utils/calculateCartTotal';
...
const CheckoutForm = ({ user }) => {
  ...
  const [cartAmoutMinus, setCartAmaountMinus] = React.useState<number>(0);

  useEffect(() => {
    const { cartTotal } = calculateCartTotal(cartItems);
    const { cartTotalMinus } = calculateCartTotalMinus(cartItems);
    setCartAmaount(parseInt(cartTotal));
    setCartAmaountMinus(parseInt(cartTotalMinus));
  }, [cartItems]);
            ...
            <div className='cart-content'>
              <ul className='single-cart'>
                {cartItems.length > 0 ? (
                  <AnimatePresence>
                    {cartItems.map((cart) => (
                      <CheckoutList key={cart.id} {...cart} onRemove={() => handleRemove(cart.id)} />
                    ))}
                  </AnimatePresence>
                ) : (
                  <>
