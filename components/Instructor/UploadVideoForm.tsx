...
const UploadVideoForm = ({ courseId }) => {
  ...
  const handleChange = (e) => {
    ...
    if (name === 'thumb') {
      ...
      if (fileSize > 2) {
        toast.error('The thumb size greater than 2 MB. Make sure less than 2 MB.', {
          style: {
      ...
      fileSize = files[0].size / 1024 / 1024;
      if (fileSize > 5) {
        toast.error('The video size greater than 5 MB. Make sure less than 5 MB.', {
          style: {
            border: '1px solid #ff0033',
            padding: '16px',
            color: '#ff0033'
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { group_name, title, video_length, is_preview, short_id, courseId } = video;
  ...
  return (
    <form onSubmit={handleSubmit}>
      <div className='row'>
        ...
        <div className='col-md-6'>
          <div className='form-group'>
            <label className='form-label fw-semibold'>Select Thumbnail Image</label>
            <input type='file' className='form-control file-control' name='thumb' onChange={handleChange} required={true} />
        ...
        <div className='col-md-6'>
          <div className='form-group'>
            ...
            <input type='file' className='form-control file-control' name='video' onChange={handleChange} />
          </div>
        </div>
        <div className='col-md-6'>
          <div className='form-group'>
            <label className='form-label fw-semibold'>Video Order Number (Ascending)</label>
            <input
              type='number'
              className='form-control'
              placeholder='Group Title'
        ...
        <div className='col-12'>
          <Button loading={loading} disabled={loading || disabled} btnText='Upload Video' btnClass='default-btn' />
        </div>
      </div>
    </form>
  );
